...
CGI::Application::Structured is based on CGI::Application and includes a number of vetted CGIApp plugins.  This project is very similar
...
Titanium takes a very light-weight approach to make running in a CGI environment very fast. CGI::Application::Structured takes a slightly different approach and aims for:
...
CGI::Application::Structured is also different in that it generates a micro-architecture and helper scripts that work within that structure to speed development and maintain project structure. The helper scripts eliminate the tedium of error-prone manual creation of controllers, templates and database mappings.

The skeleton app generated by L<cas-starter.pl> generates a base class rather than runnable module.  This is 
differen than the artifacts generated by titanium-starter.pl.  cas-starter also generates a default 
'Home' subclass and a CGI::Application::Dispatch instance that is customized to default to the 
Home subclasses generated 'index' runmode.  

L<attainum-starter.pl> also generates the script 'YourApp/script/create_controller.pl'  which will 
generate additional subclasses of your base module with a default 'index' runmode and a 
default TT template for that runmode. The provided 'YourApp/script/create_dbic_schema.pl' will generate a DBIx::Class::Schema subclass and a set of 
DBIx::Class::Result set subclasses for your custom databasse.

Finally CGI::Application::Structured aims to be as compatible as possible with L<Catalyst> which also has the plugins which were 
added to Titanium to form CGI::Application::Structured.
...
You will need to install L<CGI::Application::Structured> which provides the runtime requirements.  You will also need to install L<Module::Starter::Plugin::Attanium> which supplies the development environment.
